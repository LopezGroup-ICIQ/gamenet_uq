...
class UQTestNet2(torch.nn.Module):
    def __init__(self, 
                 features_list: list[str],
                 scaling_params: dict,                
                 dim: int,                  
        ...
        Args:
            ...
            pool (_type_, optional): Pooling Layer. Default to GraphMultisetTransformer.
        """
        super(UQTestNet2, self).__init__()
        self.dim = dim
        self.node_features_list = features_list
        self.in_features = len(features_list)
        self.scaling_params = scaling_params
        ...
        self.pma = PMA(channels = dim, 
                       ...
                       num_seeds = 1, 
                       bias = bias)
        self.mu_layer = Linear(self.dim, 1, bias=bias)
        self.std_layer = torch.nn.Sequential(
            Linear(self.dim, 1, bias=bias),
            # self.sigma,
    ...
    def forward(self, data):
        ...
        mask = (~mask).unsqueeze(1).to(dtype=out.dtype) * -1e9
        out = self.pma(x=batch_x, mask=mask)
        mu = self.mu_layer(out.squeeze(1))
        sigma = self.std_layer(out.squeeze(1))
